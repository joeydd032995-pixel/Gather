...
-- instead of scanning the whole table under ACCESS EXCLUSIVE. The scan happens
-- in the VALIDATE, which runs in migration 0007 — sqlx wraps each migration
-- file in its own transaction, so keeping the VALIDATE in a separate file lets
-- the ADD's lock be released before the (weaker-locking) validation scan. This
-- daemon runs migrations at startup before serving traffic, so the lock is
-- moot here, but it keeps the pattern correct and satisfies the migration lint.
ALTER TABLE atomic_unit_provenance
