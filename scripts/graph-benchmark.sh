...
log "seeding: ${ENTITIES} entities, ${RELATIONSHIPS} relationships, alpha=${ALPHA}"
...
BEGIN
  ...
  -- depth 2 for the same roots -- an artifact of measurement order, not of
  -- traversal cost.
  FOREACH d IN ARRAY string_to_array('${DEPTHS}', ' ')::int[] LOOP
  ...
  -- Measured. Each root is run GATHER_BENCH_REPEATS times: a p95 over only a
  -- handful of samples is just the maximum wearing a percentile's name.
  FOREACH d IN ARRAY string_to_array('${DEPTHS}', ' ')::int[] LOOP
    FOR r IN SELECT id, tier FROM bench_roots LOOP
