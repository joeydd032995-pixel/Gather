...
| `bundle-postgres.sh` / `bundle-postgres.ps1` | Yes. Builds the PostgreSQL + pgvector runtime for the desktop installer into `apps/desktop/src-tauri/resources/postgres` from pinned, checksum-verified sources, then smoke-tests it on a throwaway cluster (port 7699). Run it as a normal user: `initdb` refuses root. | CI (`build-desktop`), or your machine when packaging locally ([docs/RELEASING.md](../docs/RELEASING.md)). |
| `memory-budget.sh` | Yes: it only adds rows (a 200-file batch, a large text file, 24 MP photos) and never deletes any. Still, point it at a scratch database. | CI (`memory-budget` job, in both profiles), or your machine after changing ingest, extraction or photo code. `GATHER_MEMORY_PROFILE=low` runs it against the low-memory profile's defaults and a tighter budget. |
| `release-manifest.py` | Yes, only reads the artifacts. Writes `SHA256SUMS` and, for builds with the updater key, `latest.json`. | CI (`release` job). |
