...
| `memory-budget.sh` | Yes: it only adds rows (a 200-file batch, a large text file, 24 MP photos) and never deletes any. Still, point it at a scratch database. | CI (`memory-budget` job), or your machine after changing ingest, extraction or photo code. |
